...
/* ---------- year + build stamp ---------------------------------------------
 * The build stamp answers "am I looking at the new version?" without anyone
 * having to guess. With unhashed asset names, a stale cache is visually
 * identical to a deploy that changed nothing — this is what separates them. */
var yr = document.getElementById('yr');
if (yr) yr.textContent = new Date().getFullYear();

var buildEl = document.getElementById('buildStamp');
var b = window.GG_BUILD;
if (buildEl) {
  buildEl.textContent = b ? ('build ' + b.commit + ' · ' + b.built)
                          : 'build unstamped (config.js not generated by build.sh)';
}
console.info('[gg] ' + (b ? 'build ' + b.commit + ' · built ' + b.built
                          : 'build unstamped — config.js was not generated by build.sh'));
})();
